...
const handleNext = () => {
    if (!form.amount || parseFloat(form.amount) <= 0) {
        alert('Jumlah yang ingin ditarik harus lebih dari 0');
        return;
    }

    if (parseFloat(form.amount) > props.maxBalance) {
        alert('Jumlah penarikan melebihi saldo yang tersedia');
        return;
    }

    if (!form.description) {
        alert('Keterangan penarikan harus diisi');
        return;
    }

    if (form.method === 'Non-Tunai') {
        if (!form.bank_name || !form.account_number || !form.account_name) {
            alert('Data bank harus diisi lengkap untuk metode Non-Tunai');
            return;
        }
    }

    router.get(route('user.simpanan.withdraw.statement'), {
        amount: form.amount,
